...
/*
template <class ComplexType> void LatticeFFT::cfft(Lattice<DComplex>& cLattice,
		     const Vector<Bool>& whichAxes, const Bool toFrequency) {
  const uInt ndim = cLattice.ndim();
  DebugAssert(ndim > 0, AipsError);
  DebugAssert(ndim == whichAxes.nelements(), AipsError);
...
*/
template <class ComplexType> void LatticeFFT::cfft(Lattice<ComplexType>& cLattice, const Bool toFrequency) {
  const Vector<Bool> whichAxes(cLattice.ndim(), True);
  LatticeFFT::cfft(cLattice, whichAxes, toFrequency);
}

template <class ComplexType> void LatticeFFT::cfft(Lattice<DComplex>& cLattice, const Bool toFrequency) {
  const Vector<Bool> whichAxes(cLattice.ndim(), True);
  LatticeFFT::cfft(cLattice, whichAxes, toFrequency);
}

template <class ComplexType> void LatticeFFT::rcfft(Lattice<ComplexType>& out, const Lattice<typename NumericTraits<ComplexType>::ConjugateType>& in,
  ...
  TempLattice<typename NumericTraits<ComplexType>::ConjugateType> inlocal(TiledShape(in.shape(), tileShape));
  inlocal.put(in.get());
  FFTServer<typename NumericTraits<ComplexType>::ConjugateType,ComplexType> ffts;
...
// ----------------MYRCFFT--------------------------------------
//
template <class ComplexType> void LatticeFFT::myrcfft(Lattice<ComplexType>& out, const Lattice<typename NumericTraits<ComplexType>::ConjugateType>& in,
		     const Vector<Bool>& whichAxes, const Bool doShift){
...
template <class ComplexType> void LatticeFFT::crfft(Lattice<typename NumericTraits<ComplexType>::ConjugateType>& out, Lattice<ComplexType>& in,
		       const Vector<Bool>& whichAxes, const Bool doShift, 
		       Bool doFast){
  const uInt ndim = in.ndim();
  DebugAssert(ndim > 0, AipsError);
...
template <class ComplexType> void LatticeFFT::crfft(Lattice<typename NumericTraits<ComplexType>::ConjugateType>& out, const Lattice<ComplexType>& in,
		       const Bool doShift, Bool doFast){
